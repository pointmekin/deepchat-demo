const genAI = require('genAI') // Assuming genAI is the library used for the generative model

describe('run', () => {
  it('should generate content based on prompt', async () => {
    // Mocking the generative model
    const mockModel = {
    ...
    // Mocking the genAI.getGenerativeModel function
    jest.spyOn(genAI, 'getGenerativeModel').mockReturnValue(mockModel)
    ...
    expect(mockModel.generateContent)
      .toHaveBeenCalledWith(`Summarize what's happening to this stock's earnings per share based on the data provided:
  data: """
  Revenue
Cost Of Goods Sold
Gross Profit
...
2,239.32
  """`)
    ...
    // Verifying the console.log output
    expect(console.log).toHaveBeenCalledWith('Generated content')
  })
})
describe('run2', () => {
  it('should generate content based on prompt', async () => {
